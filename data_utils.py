...
def prepare_data_domainNet(cfg, data_base_path):
    ...
    train_set = [infograph_trainset, painting_trainset, quickdraw_trainset, real_trainset, sketch_trainset]
    test_set = [clipart_testset]*5
    #test_set = [sketch_trainset+sketch_testset]
...
def prepare_data_office(cfg, data_base_path):
    ...
    train_set = [amazon_trainset, caltech_trainset,webcam_trainset]
...
def prepare_data_office_partition_train(cfg, data_base_path):
    ...
    for dataset in [amazon_trainset, caltech_trainset, dslr_trainset,webcam_trainset]:
        for i in range(n_clients):
            train_data_num_list.append(len(dataset[i]))
            train_set.append(dataset[i])
    for dataset in [amazon_testset,caltech_trainset,dslr_testset,webcam_testset]:
        for i in range(n_clients):
            test_data_num_list.append(len(dataset[i]))
            test_set.append(dataset[i])
    print("train_data_num_list:", train_data_num_list)
...
def prepare_data_officehome(cfg, data_base_path):
    data_base_path = data_base_path
    transform_train = transforms.Compose([
        transforms.Resize([256, 256]),
        transforms.RandomHorizontalFlip(),
    ...
    # art
    art_trainset = OfficeHomeDataset(data_base_path, 'Art', transform=transform_train)
    lab2cname = art_trainset.lab2cname
    classnames = art_trainset.classnames
    art_trainset = art_trainset.data_detailed
    art_testset = OfficeHomeDataset(data_base_path, 'Art', transform=transform_test, train=False).data_detailed

    # clipart
    clipart_trainset = OfficeHomeDataset(data_base_path, 'Clipart', transform=transform_train).data_detailed
    clipart_testset = OfficeHomeDataset(data_base_path, 'Clipart', transform=transform_test, train=False).data_detailed

    # product
    product_trainset = OfficeHomeDataset(data_base_path, 'Product', transform=transform_train).data_detailed
    product_testset = OfficeHomeDataset(data_base_path, 'Product', transform=transform_test, train=False).data_detailed

    # real_world
    real_world_trainset = OfficeHomeDataset(data_base_path, 'Real World', transform=transform_train).data_detailed
    real_world_testset = OfficeHomeDataset(data_base_path, 'Real World', transform=transform_test,
                                           train=False).data_detailed

    train_data_num_list = [len(art_trainset), len(clipart_trainset), len(product_trainset), len(real_world_trainset)]
    test_data_num_list = [len(art_testset), len(clipart_testset), len(product_testset), len(real_world_testset)]
    print("train_data_num_list:", train_data_num_list)
    print("test_data_num_list:", test_data_num_list)

    train_set = [art_trainset,clipart_trainset,product_trainset,real_world_trainset]
    test_set = [art_testset,clipart_testset,product_testset,real_world_testset]   # 使用clipart作为测试集，与DomainNet保持一致

    return train_set, test_set, classnames, lab2cname

def prepare_data_officehome_partition_train(cfg, data_base_path):
    data_base_path = data_base_path
    ...
    # Art
    print("Art: ")
    net_dataidx_map_train, net_dataidx_map_test = Dataset_partition_officehome(data_base_path, 'Art', cfg.DATASET.BETA,
                                                                               n_parties=n_clients,
                                                                               min_require_size=min_require_size)
    art_trainset = [[] for i in range(n_clients)]
    art_testset = [[] for i in range(n_clients)]
    for i in range(n_clients):
        art_trainset[i] = OfficeHomeDataset_sub(data_base_path, 'Art', net_dataidx_map_train[i],
                                                transform=transform_train)
        art_testset[i] = OfficeHomeDataset_sub(data_base_path, 'Art', net_dataidx_map_test[i], train=False,
                                               transform=transform_test).data_detailed
        lab2cname = art_trainset[i].lab2cname
        classnames = art_trainset[i].classnames
        art_trainset[i] = art_trainset[i].data_detailed

    # Clipart
    print("Clipart: ")
    net_dataidx_map_train, net_dataidx_map_test = Dataset_partition_officehome(data_base_path, 'Clipart',
                                                                               cfg.DATASET.BETA, n_parties=n_clients,
                                                                               min_require_size=min_require_size)
    clipart_trainset = [[] for i in range(n_clients)]
    clipart_testset = [[] for i in range(n_clients)]
    for i in range(n_clients):
        clipart_trainset[i] = OfficeHomeDataset_sub(data_base_path, 'Clipart', net_dataidx_map_train[i],
                                                    transform=transform_train).data_detailed
        clipart_testset[i] = OfficeHomeDataset_sub(data_base_path, 'Clipart', net_dataidx_map_test[i], train=False,
                                                   transform=transform_test).data_detailed

    # Product
    print("Product: ")
    net_dataidx_map_train, net_dataidx_map_test = Dataset_partition_officehome(data_base_path, 'Product',
                                                                               cfg.DATASET.BETA, n_parties=n_clients,
                                                                               min_require_size=min_require_size)
    product_trainset = [[] for i in range(n_clients)]
    product_testset = [[] for i in range(n_clients)]
    for i in range(n_clients):
        product_trainset[i] = OfficeHomeDataset_sub(data_base_path, 'Product', net_dataidx_map_train[i],
                                                    transform=transform_train).data_detailed
        product_testset[i] = OfficeHomeDataset_sub(data_base_path, 'Product', net_dataidx_map_test[i], train=False,
                                                   transform=transform_test).data_detailed

    # Real_World
    print("Real_World: ")
    net_dataidx_map_train, net_dataidx_map_test = Dataset_partition_officehome(data_base_path, 'Real World',
                                                                               cfg.DATASET.BETA, n_parties=n_clients,
                                                                               min_require_size=min_require_size)
    real_world_trainset = [[] for i in range(n_clients)]
    real_world_testset = [[] for i in range(n_clients)]
    for i in range(n_clients):
        real_world_trainset[i] = OfficeHomeDataset_sub(data_base_path, 'Real World', net_dataidx_map_train[i],
                                                       transform=transform_train).data_detailed
        real_world_testset[i] = OfficeHomeDataset_sub(data_base_path, 'Real World', net_dataidx_map_test[i],
                                                      train=False, transform=transform_test).data_detailed
    ...
    for dataset in [art_trainset, clipart_trainset, product_trainset, real_world_trainset]:
        for i in range(n_clients):
            train_data_num_list.append(len(dataset[i]))
            train_set.append(dataset[i])

    for dataset in [art_testset, clipart_testset, product_testset, real_world_testset]:
        for i in range(n_clients):
            test_data_num_list.append(len(dataset[i]))
            test_set.append(dataset[i])
...
class OfficeHomeDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        ...
        if train:
            path = os.path.join(self.base_path, 'OfficeHome/{}_train.pkl'.format(site))
            self.paths, self.label = np.load(path, allow_pickle=True)
        else:
            path = os.path.join(self.base_path, 'OfficeHome/{}_test.pkl'.format(site))
            self.paths, self.label = np.load(path, allow_pickle=True)


        self.site_domain = {'Art': 0, 'Clipart': 1, 'Product': 2, 'Real World': 3}
        self.domain = self.site_domain[site]

        # 类别名称到标签的映射
        self.lab2cname = {
            'Alarm_Clock':0, 'Backpack':1, 'Batteries':2, 'Bed':3, 'Bike':4, 'Bottle':5, 'Bucket':6,
            'Calculator':7, 'Calendar':8, 'Candles':9, 'Chair':10, 'Clipboards':11, 'Computer':12,
            'Couch':13, 'Curtains':14, 'Desk_Lamp':15, 'Drill':16, 'Eraser':17, 'Exit_Sign':18,
            'Fan':19, 'File_Cabinet': 20, 'Flipflops': 21, 'Flowers': 22, 'Folder': 23, 'Fork': 24,
            'Glasses': 25, 'Hammer': 26, 'Helmet': 27, 'Kettle': 28, 'Keyboard': 29, 'Knives': 30,
            'Lamp_Shade': 31, 'Laptop': 32, 'Marker': 33, 'Monitor': 34, 'Mop': 35, 'Mouse': 36,
            'Mug': 37, 'Notebook': 38, 'Oven': 39, 'Pan': 40, 'Paper_Clip': 41, 'Pen': 42, 'Pencil': 43,
            'Postit_Notes': 44, 'Printer': 45, 'Push_Pin': 46, 'Radio': 47, 'Refrigerator': 48, 'Ruler': 49,
            'Scissors': 50, 'Screwdriver': 51, 'Shelf': 52, 'Sink': 53, 'Sneakers': 54, 'Soda': 55,
            'Speaker': 56, 'Spoon': 57, 'TV': 58, 'Table': 59, 'Telephone': 60, 'ToothBrush': 61,
            'Toys': 62, 'Trash_Can': 63, 'Webcam': 64
        }

        self.classnames = {"Alarm_Clock","Backpack","Batteries","Bed","Bike","Bottle","Bucket","Calculator","Calendar","Candles","Chair","Clipboards","Computer","Couch","Curtains",
    "Desk_Lamp","Drill","Eraser","Exit_Sign","Fan","File_Cabinet","Flipflops","Flowers","Folder","Fork","Glasses","Hammer","Helmet","Kettle",
    "Keyboard","Knives","Lamp_Shade","Laptop","Marker","Monitor","Mop","Mouse","Mug","Notebook",
    "Oven","Pan","Paper_Clip","Pen","Pencil","Postit_Notes","Printer","Push_Pin","Radio","Refrigerator",
    "Ruler","Scissors", "Screwdriver",   "Shelf","Sink","Sneakers","Soda","Speaker","Spoon",
    "Table","Telephone","ToothBrush", "Toys","Trash_Can","TV","Webcam" }
    ...
    def __getitem__(self, idx):
        ...
        # Return Datum object instead of tuple
...
class DomainNetDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        ...
        self.domain = self.site_domian[site]
        self.lab2cname = {'bird': 0, 'feather': 1, 'headphones': 2, 'ice_cream': 3, 'teapot': 4, 'tiger': 5, 'whale': 6,
                          'windmill': 7, 'wine_glass': 8, 'zebra': 9}
        self.classnames = {'bird', 'feather', 'headphones', 'ice_cream', 'teapot', 'tiger', 'whale', 'windmill',
                           'wine_glass', 'zebra'}
        self.target = [self.lab2cname[text] for text in self.label]
        if train:
            print('Counter({}_train data:)'.format(site), Counter(self.target))
...
def Dataset_partition_officehome(base_path, site, beta, split_test=True, n_parties=3, min_require_size=2):
    min_size = 0
    K = 65  # OfficeHome 数据集有 65 个类别
    # np.random.seed(2023)

    train_path = os.path.join(base_path, 'OfficeHome', f'{site}_train.pkl')
    test_path = os.path.join(base_path, 'OfficeHome', f'{site}_test.pkl')
...
def Adjust_test_dataset_officehome(site, class_ratio):
    c_num = 65  # OfficeHome 有 65 个类别
     # 动态生成标签映射
    label_dict = {
        'Alarm_Clock': 0, 'Backpack': 1, 'Batteries': 2, 'Bed': 3, 'Bike': 4, 'Bottle': 5, 'Bucket': 6,
        'Calculator': 7, 'Calendar': 8, 'Candles': 9, 'Chair': 10, 'Clipboards': 11, 'Computer': 12,
    ...
    _, test_text_labels =np.load('data/OfficeHome/{}_test.pkl'.format(site), allow_pickle=True)
    test_labels = np.asarray([label_dict[text] for text in test_text_labels])

    # 计算当前测试集的类别分布
    unq, unq_cnt = np.unique(test_labels, return_counts=True)
    test_class_ratio = {unq[i]: unq_cnt[i] for i in range(len(unq))}

    # 根据 class_ratio 调整每个类别的样本数量
    times = [test_class_ratio[x] / class_ratio[x] for x in range(c_num)]
    min_time = min(times)
    right_class_num = [int(min_time * class_ratio[x]) for x in range(c_num)]
...
class OfficeHomeDataset_sub(Dataset):
    def __init__(self, base_path, site, net_dataidx_map, train=True, transform=None):
        self.base_path = base_path
        self.site = site
        self.transform = transform
       ...
       'Couch': 13, 'Curtains': 14, 'Desk_Lamp': 15, 'Drill': 16, 'Eraser': 17, 'Exit_Sign': 18,
        ...
        self.classnames = {'Alarm_Clock', 'Backpack', 'Batteries', 'Bed', 'Bike', 'Bottle', 'Bucket',
        'Calculator', 'Calendar', 'Candles', 'Chair', 'Clipboards', 'Computer'
       'Couch', 'Curtains', 'Desk_Lamp', 'Drill', 'Eraser', 'Exit_Sign',
        'Fan', 'File_Cabinet', 'Flipflops', 'Flowers', 'Folder', 'Fork',
        'Glasses', 'Hammer', 'Helmet', 'Kettle', 'Keyboard', 'Knives',
       'Lamp_Shade', 'Laptop', 'Marker', 'Monitor', 'Mop', 'Mouse',
        'Mug', 'Notebook', 'Oven', 'Pan', 'Paper_Clip', 'Pen', 'Pencil',
        'Postit_Notes', 'Printer', 'Push_Pin', 'Radio', 'Refrigerator', 'Ruler',
        'Scissors', 'Screwdriver', 'Shelf', 'Sink', 'Sneakers', 'Soda',
        'Speaker', 'Spoon', 'TV', 'Table', 'Telephone', 'ToothBrush',

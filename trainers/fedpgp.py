...
class PromptLearner(nn.Module):
    ...
    def forward(self):
        ...
        if self.class_token_position == "end":
            # 检查并打印维度以便调试
            #print(f"prefix shape: {prefix.shape}, ctx shape: {ctx.shape}, suffix shape: {suffix.shape}")
            #print(f"prefix shape: {prefix.shape}, sigma shape: {sigma.shape}, suffix shape: {suffix.shape}")
            #print(f"prefix shape: {prefix.shape}, BA shape: {BA.shape}, suffix shape: {suffix.shape}")
...
class VisualDynamicFusion(nn.Module):
    ...
    def forward(self, x_orig, x_glo, x_loc):
        ...
        # 3. 计算权重 (Softmax over 3 components)
        # 拼接分数: [Batch, 3]
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        # 融合特征 (Fusion): 使用动态门控网络
        image_features= self.visual_fusion(image_features_0, image_features_glo, image_features_loc)
        #image_features=(image_features_0+image_features_glo+image_features_loc)/3

        # 打印权重分布 (取Batch均值)
        #print(f"Fusion Weights: Orig={fusion_weights[:,0].mean().item():.4f}, Glo={fusion_weights[:,1].mean().item():.4f}, Loc={fusion_weights[:,2].mean().item():.4f}")
        ...
        if self.training == True:
            ...
            #logits_0 = logit_scale * image_features_0@text_features_0.t()
...
class FedPGP(TrainerX):
    ...
    def build_model(self):
        ...
        for name, param in self.model.named_parameters():
            # 允许 prompt_learner, glo_adapter, loc_adapter, visual_fusion 参数更新
        ...
        self.register_model("FedPGP", self.model, self.optim, self.sched)
    ...
    def forward_backward(self, batch):
        ...
        loss_summary = {
            ...
            "reg_loss": reg_loss.item() if prec != "amp" else 0,  # 添加正则化损失到摘要
        }
